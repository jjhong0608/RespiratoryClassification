...
def test_training_config_parses_imbalance_options(tmp_path: Path) -> None:
    cfg = {
        "run_name": "x",
        "seed": 1,
        "device": "cpu",
    ...
    assert parsed.imbalance.auto_pos_weight is True
    assert parsed.imbalance.pos_weight is None
    assert parsed.imbalance.sampler == "weighted_random"
